...
`Unity-Data-Shards/Assets/root/` with Unity's UPM CLI, verifies the resulting archive
...
```bash
gh secret set UPM_SERVICE_ACCOUNT_KEY_ID     --repo YOUR_GITHUB_USERNAME/YOUR_REPO
gh secret set UPM_SERVICE_ACCOUNT_KEY_SECRET --repo YOUR_GITHUB_USERNAME/YOUR_REPO
gh secret set UPM_ORG_ID                     --repo YOUR_GITHUB_USERNAME/YOUR_REPO
```
